...
class RegisterViewModel : ViewModel() {
    ...
    fun sendRegisterCode() {
        ...
        viewModelScope.launch {
            ...
            try {
                val res = ServiceLocator.apiService.sendRegisterEmail(email)
    ...
    fun register() {
        ...
        when {
            s.username.isBlank() -> {
                _state.update { it.copy(error = "请输入用户名") }; return
            }
            s.email.isBlank() -> {
                _state.update { it.copy(error = "请输入邮箱") }; return
            }
            s.code.isBlank() -> {
                _state.update { it.copy(error = "请输入邮箱验证码") }; return
            }
            s.password.length < 6 -> {
        ...
        viewModelScope.launch {
            ...
            try {
                val res = ServiceLocator.apiService.register(
                ...
                if (res.effectiveStatus == 200) {
                    ...
                    _state.update {
                        it.copy(
                            loading = false,
                            error = res.effectiveMessage ?: "注册失败（${res.effectiveStatus}）"
                        )
                    }
                }
            } catch (e: Exception) {
